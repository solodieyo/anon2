...
class StatisticRepository(BaseRepository):
    ...
    async def get_senders_top(self, user_id: int):
        subquery = select(
            ...
            func.row_number().over(order_by=[func.count(Message.id).desc(), User.id]).label('place')
        ).select_from(User).join(Message, Message.from_user_id == User.id).group_by(User.id).subquery()

        result_top = await self.session.execute(
            select(subquery).order_by(subquery.c.place).limit(10)
        )

        result_user_info = await self.session.execute(
            select(subquery.c.place, subquery.c.message_count).where(subquery.c.id == user_id)
        )
        ...
        return RatingDTO(
            ...
            from_user_place=user_info[0] if user_info else "xxx",
            from_user_count=user_info[1] if user_info else 0
        )
    ...
    async def get_top_place(self, user_pk: int):
        ...
        #     func.count(Message.id).label('message_count'),
        #     func.row_number().over(order_by=[func.count(Message.id).desc(), User.id]).label('place')
        # ).select_from(User).outerjoin(Message, Message.from_user_id == User.id).group_by(User.id).subquery()
        #
        # query = select(subquery.c.place, subquery.c.message_count).select_from(subquery).filter(
        #     subquery.c.id == user_pk)
        #
        # result_place = await self.session.execute(query)

        result = await self.session.execute(
            select(func.dense_rank().over(order_by=User.user_id.desc()).label('place'))
    ...
    async def get_getters_top(self, user_id: int):
        subquery = select(
            ...
            func.count(Message.id).label('message_count'),
            func.row_number().over(order_by=[func.count(Message.id).desc(), User.id]).label('place')
        ).select_from(User).join(Message, Message.to_user_id == User.id).group_by(User.id).subquery()
